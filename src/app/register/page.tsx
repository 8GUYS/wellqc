...
export default function RegisterPage() {
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center px-4">
      <div className="w-full max-w-lg rounded-2xl border border-slate-800 bg-slate-900/80 p-8 shadow-2xl shadow-cyan-500/5 backdrop-blur-sm">
        <div className="mb-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <label className="flex items-start gap-3 rounded-xl border border-slate-700 bg-slate-950/60 p-3 text-sm text-slate-300">
            <input
              type="checkbox"
              checked={form.acceptedNda}
              onChange={(event) => setForm((prev) => ({ ...prev, acceptedNda: event.target.checked }))}
              className="mt-1 h-4 w-4 rounded border-slate-500 bg-slate-900"
            />
            <span>I accept the confidentiality agreement and NDA terms.</span>
          </label>
