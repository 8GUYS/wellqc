...
export function PaymentModal({
  ...
  // instead of in an effect, per React's recommended pattern for syncing
  // state to a prop change — avoids an extra render/flicker.
  ...
  // Update default plan if prop changes — same render-time adjustment pattern.
  ...
  // NOTE: runPaystackCheckout only ever executes in response to a user click
  // (via handlePaystackCheckout / handleQuickDemoLogin above), never during
  // render. The two eslint-disable comments below are scoped to lines that
  // the React Compiler lint plugin flags conservatively because it can't
  // always trace call-site purity through nested handler functions.
  const runPaystackCheckout = async () => {
    ...
    try {
      ...
      if (data.authorizationUrl) {
        // eslint-disable-next-line react-hooks/immutability -- navigation triggered only from a click handler, never during render
